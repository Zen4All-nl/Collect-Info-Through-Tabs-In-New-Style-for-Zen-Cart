...
switch ($action) {
  ...
  case 'update_product':
    ...
    $new_id = 0;
    $discount_cnt = 0;
    if (array($_POST['discount_qty']) && !empty($_POST['discount_qty'])) {
      for ($i = 1, $n = sizeof($_POST['discount_qty']); $i <= $n; $i++) {
        if ($_POST['discount_qty'][$i] > 0) {
          ...
          $db->Execute("INSERT INTO " . TABLE_PRODUCTS_DISCOUNT_QUANTITY . " (discount_id, products_id, discount_qty, discount_price)
                        VALUES (" . (int)$new_id . ", " . (int)$products_id . ", '" . zen_db_input($_POST['discount_qty'][$i]) . "', '" . zen_db_input($_POST['discount_price'][$i]) . "')");
          $discount_cnt++;
        } else {
          loop;
        }
